...
export default function AgentsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (usersResponse.ok) {
        ...
        const rescueAgents = users.filter((u: Agent) => u.roles.includes('AMBULANCIER'));
        setAgents(rescueAgents);
      }

      const shiftsResponse = await fetch('/api/rescue/shifts');
      if (shiftsResponse.ok) {
        const shiftsData = await shiftsResponse.json();
        setShifts(shiftsData);
      }

      const patrolsResponse = await fetch('/api/rescue/patrols');
      if (patrolsResponse.ok) {
        const patrolsData = await patrolsResponse.json();
        setPatrols(patrolsData);
      }
    } catch (error) {
      console.error('Erreur lors du chargement des ambulanciers:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getAgentStatus = (agentId: string) => {
    ...
    if (activePatrol) return { statut: "En intervention", icon: "🔵" };
  ...
  const getAgentPatrol = (agentId: string) => {
    ...
    return `Intervention ${patrol.sector}`;
  };
  ...
  return (
    <div className="page-enter">
      <PageHeader
        brand="ClearRescue"
        title="Ambulanciers"
        subtitle="Liste des ambulanciers et leur statut en temps réel"
      />

      <section className="mb-8">
        <h2 className="mb-4 text-lg font-bold text-ink">Ambulanciers</h2>
        {loading ? (
          ...
          <div className="panel-soft p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-extrabold text-ink">
                  {agents.filter(a => getAgentStatus(a.id).statut === "En intervention").length}
                </p>
              </div>
              <span className="text-2xl">🔵</span>
            </div>
