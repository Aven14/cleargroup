...
export default function AlertesPage() {
  ...
  const handleAcceptAlerte = async (id: string) => {
    try {
      const response = await fetch(`/api/rescue/alerts/${id}/respond`, {
        method: 'POST',
      });
      if (response.ok) {
        await loadAlertes();
  ...
  const handleCloseAlerte = async (id: string) => {
    try {
      const response = await fetch(`/api/rescue/alerts/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="page-enter">
      <PageHeader
        brand="ClearRescue"
        title="Alertes"
        subtitle="Gestion des alertes médicales en temps réel"
      />
        ...
        {showCreateForm && (
          <div className="mt-4 panel-soft p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="label-caps block mb-2">Nombre d&apos;ambulanciers requis</label>
                <input
                  type="number"
                  className="input-field w-full"
              ...
              <div key={alerte.id} className={`p-6 border rounded-lg ${alerte.active ? 'bg-white border-red-400' : 'bg-white border-gray-200 opacity-60'}`}>
                ...
                <div className="flex items-center justify-between mb-4">
                  <span className="text-sm text-muted">
                    {alerte.responses.length} / {alerte.agentsRequested} ambulanciers
                  </span>
                ...
                {alerte.responses.length > 0 && (
                  <div className="mb-4 p-4 bg-primary-light/20 rounded">
                    <p className="text-xs text-muted mb-2">Ambulanciers acceptés</p>
