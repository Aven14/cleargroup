  ...
  return (
    <div className="page-enter">
      <PageHeader
        brand="ClearRescue"
        title="Planning"
        subtitle="Gestion du planning et des événements médicaux"
      />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {evenements
            ...
            <div key={evenement.id} className="panel-soft p-6">
              ...
              <div className="mb-4">
                <p className="text-xs text-muted mb-2">Ambulanciers assignés :</p>
                {evenement.confirmedAgents.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {evenement.confirmedAgents.map((agent, idx) => (
                  ...
                  <p className="text-sm text-muted">Aucun ambulancier assigné</p>
                )}
              </div>
              {currentUser && currentUser.roles.includes('AMBULANCIER') && (
                <div>
                  {evenement.confirmedAgents.includes(`${currentUser.firstname} ${currentUser.lastname}`) ? (
                    <button
