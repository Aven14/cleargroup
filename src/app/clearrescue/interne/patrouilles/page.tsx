...
export default function PatrouillesPage() {
  ...
  const loadPatrouilles = async () => {
    try {
      const response = await fetch('/api/rescue/patrols');
      if (response.ok) {
  ...
  const handleEndPatrouille = async (id: string) => {
    try {
      const response = await fetch(`/api/rescue/patrols/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleJoinPatrouille = async (id: string) => {
    try {
      const response = await fetch(`/api/rescue/patrols/${id}/join`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      if (response.ok) {
  ...
  const handleLeavePatrouille = async (id: string) => {
    try {
      const response = await fetch(`/api/rescue/patrols/${id}/leave`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      if (response.ok) {
  ...
  return (
    <div className="page-enter">
      <PageHeader
        brand="ClearRescue"
        title="Interventions"
        subtitle="Gestion des interventions médicales et des missions en cours"
      />
        ...
        {showCreateForm && (
          <div className="mb-4 panel-soft p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block mb-2 text-sm font-medium text-muted">Ambulance</label>
                <input
                  type="text"
                  className="input-field w-full"
              ...
              <div>
                ...
                >
                  <option>Intervention médicale</option>
                  <option>Transport sanitaire</option>
                  <option>Événement</option>
                  <option>Escorte médicale</option>
                </select>
              </div>
              <div>
                <label className="block mb-2 text-sm font-medium text-muted">Nombre max d&apos;ambulanciers (max 8)</label>
                <input
                  type="number"
                  min="1"
                  max="8"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {activePatrouilles.map((patrouille) => (
              <div key={patrouille.id} className="p-4 border rounded-lg bg-white border-green-400">
                ...
                <div className="mb-3">
                  <p className="text-xs text-muted mb-1">Ambulanciers ({patrouille.agents.length}/{patrouille.maxAgents})</p>
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  ...
                  <div>
                    <p className="text-xs text-muted">Ambulance</p>
                ...
                {currentUser && currentUser.roles.includes('AMBULANCIER') && (
                  <div className="mb-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {completedPatrouilles.map((patrouille) => (
              <div key={patrouille.id} className="p-4 border rounded-lg bg-white border-gray-200 opacity-80">
                ...
                <div className="mb-3">
                  <p className="text-xs text-muted mb-1">Ambulanciers ({patrouille.agents.length})</p>
                  <div className="flex flex-wrap gap-1">
                    {patrouille.agents.map((a, idx) => (
