...
export default function PlanningPage() {
  ...
  const [currentUser, setCurrentUser] = useState<Mechanic | null>(null);
  ...
  const handleConfirmMechanic = (evenementId: string, mechanicName: string) => {
    setEvenements(evenements.map(e => {
      if (e.id === evenementId) {
        if (e.confirmedMechanics.includes(mechanicName)) {
          return {
            ...e,
            confirmedMechanics: e.confirmedMechanics.filter(a => a !== mechanicName)
          };
        } else {
          return {
            ...e,
            confirmedMechanics: [...e.confirmedMechanics, mechanicName]
          };
        }
      }
      return e;
  ...
  return (
    <div className="page-enter">
      <PageHeader
        brand="ClearDP"
        title="Planning"
        subtitle="Gestion du planning et des événements de dépannage"
      />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {evenements
            ...
            <div key={evenement.id} className="panel-soft p-6">
              ...
              <div className="mb-4">
                <p className="text-xs text-muted mb-2">Mécaniciens assignés :</p>
                {evenement.confirmedMechanics.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {evenement.confirmedMechanics.map((mechanic, idx) => (
                  ...
                  <p className="text-sm text-muted">Aucun mécanicien assigné</p>
                )}
              </div>
              {currentUser && currentUser.roles.includes('MECANICIEN') && (
                <div>
                  {evenement.confirmedMechanics.includes(`${currentUser.firstname} ${currentUser.lastname}`) ? (
                    <button
                      onClick={() => handleConfirmMechanic(evenement.id, `${currentUser.firstname} ${currentUser.lastname}`)}
